...
u16 Left1Max=1430,Y1Max=1415,Left2Max=1500,MiddleMax=1600,Right2Max=1410,Y2Max=1200,Right1Max=1625;
  
void ReadAD(void)
{
    //已经含有均值滤波功能
    AD[0]=(AD[0]*5+ad_ave(ADC0, AD8,WEI, AVE)*6)/11;
    AD[1]=(AD[1]*5+ad_ave(ADC0, AD9,WEI, AVE)*6)/11;
    AD[2]=(AD[2]*5+ad_ave(ADC0, AD12 ,WEI, AVE)*6)/11;
    AD[3]=(AD[3]*5+ad_ave(ADC0, AD13 ,WEI, AVE)*6)/11;
    AD[4]=(AD[4]*5+ad_ave(ADC1, AD14,WEI, AVE)*6)/11;
    AD[5]=(AD[5]*5+ad_ave(ADC1, AD15,WEI, AVE)*6)/11;  
    AD[6]=(AD[6]*5+ad_ave(ADC0, AD14,WEI, AVE)*6)/11;  
    AD[7]=(AD[7]*5+ad_ave(ADC0, AD15,WEI, AVE)*6)/11;
    ...
    Left1  = AD[7]*1500/Left1Max;
    Y1     = AD[6]*1000/Y1Max;
    Left2  = AD[5]*1500/Left2Max;
    Middle = AD[4]*2200/MiddleMax;
    Right2 = AD[2]*1500/Right2Max;
    Y2     = AD[1]*1000/Y2Max;
    Right1 = AD[0]*1500/Right1Max;
...
{       
  ...
  if(Left1>=100&&Right1>=100) {
    OffsetKeep=Offset;
    OffsetLeftKeep=Left;
    OffsetRightKeep=Right;
  }
  
  if(Left1<100&&OffsetKeep>0) {
      Offset_I  = OffsetKeep+(OffsetLeftKeep-Left)*3;
  } else {
      if(Right1<100&&OffsetKeep<0) {
...
if( PB[1] == 1 ) {
  ...
  if(d_ramp==1&&t_ramp>dt_ramp+800)   //下坡延时清标志位
  {
      flag_ramp=0;
      d_ramp=0;
  }
